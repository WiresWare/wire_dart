...
class TestWireMiddleware extends WireMiddleware {
  ...
  void onSend(String? signal, [data, scope]) {
    print(
        '> TestWireMiddleware -> onRemove: signal = ${signal} | $data | $scope');
  }
}
...
void main() {
  group('1. Subscription tests', () {
    ...
    WireListener<String> listener_string = (String? data, wid) => {
          print(
              '> Response on ${Wire.get(wireId: wid).single!.signal} with STRING data: ${data}')
        };

    WireListener<bool> listener_boolean = (bool? data, wid) => {
          print(
              '> Response on ${Wire.get(wireId: wid).single!.signal} with BOOLEAN data: ${data}')
        };

    WireListener listener_dynamic = (data, wid) => {
          print(
              '> Response on ${Wire.get(wireId: wid).single!.signal} with DYNAMIC data: ${data}')
        };
    ...
    setUp(() {
      ...
      Wire.add(SCOPE, SIGNAL,
          listener_dynamic); // Dynamic listener will react on any signal regardless attached data types
    ...
    test('1.4 Counter Signal', () {
      Wire.add(SCOPE, SIGNAL_COUNTER, (dynamic _, wid) {
  ...
  group('2. Purge and remove tests', () {
    ...
    test('2.2 Remove', () {
      Wire.add(SCOPE, SIGNAL, listener);
      Wire.add(SCOPE, SIGNAL, (dynamic wire, data) => {});
      Wire.remove(SIGNAL);
      expect(Wire.has(signal: SIGNAL), isFalse);
      expect(Wire.get(signal: SIGNAL), isEmpty);
    });
